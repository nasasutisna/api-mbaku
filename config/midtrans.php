...
class Midtrans implements MidtransFactory
{
    ...
    private static function clientRequest($url, $type, $data = null)
    {
        try {
            ...
            $request = $client->request($type, $url, [
                'headers' => self::header(),
                'verify' => dirname(_FILE_) . '/cert/cacert.pem',
                'json' => $data
            ]);
    ...
    public static function getSnapTransaction($body)
    {
        $endpoint = self::snapChargeUrl();
    ...
    {
        $endpoint = self::snapChargeUrl() . '/' . $order_id . '/status';
        return self::clientRequest($endpoint, 'GET');
    }

    public static function snapStatus($order_id)
    {
